...
def validate(doc, method=None):
    set_tax_withholding(doc)

def set_tax_withholding(doc):
    if not doc.apply_tds:
        return
    if doc.apply_tds and not doc.get("tax_withholding_category"):
        doc.tax_withholding_category = frappe.db.get_value(
            "Supplier", doc.supplier, "tax_withholding_category"
        )

    if not (doc.tax_withholding_category and doc.item_wise_tds):
        return

    if doc.item_wise_tds:
        ...
        if len(item_wise_tax_withheld):
            for i in item_wise_tax_withheld:
                doc.append("tax_withholding_details", i)


        if not len(tax_withholding_details):
            return
        accounts = []
        for t in tax_withholding_details:
            doc.allocate_advance_tds(t, advance_taxes)

            for d in doc.taxes:
                if d.account_head == t.get("account_head"):
                    d.update(t)

                accounts.append(d.account_head)

            if not accounts or t.get("account_head") not in accounts:
                doc.append("taxes", t)

        to_remove = [
            d
            for d in doc.taxes
            if not d.tax_amount and d.account_head == t.get("account_head")
            for t in tax_withholding_details
        ]
